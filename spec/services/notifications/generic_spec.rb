...
module Notifications
  RSpec.describe Generic do
    ...
    describe '.notify' do
      ...
      it 'prints the output, prints the grouping by date and how many items were processed,' \
           ' by default excludes upgraded items' do
      ...
      context 'when upgraded items are included' do
        before { allow(service).to receive(:notify_upgraded_items?).and_return(true) }

        it 'includes upgraded items' do
          output = capture_stdout { notify }
          expected_strings = [service.summary, service.class::APP_NAME, *service.grouped_items.keys.map(&:to_s)]
          expect(output.scan(/#{Regexp.union(expected_strings)}/)).to match_array(expected_strings)
          output = output.split("\n")
